...
title: Adding new posts in Reguide theme
...
ogImage: ""
description:
  Some rules & recommendations for creating or adding new posts using Reguide
  theme.
---

Here are some rules/recommendations, tips & ticks for creating new posts in Reguide blog theme.
...
Title and description (excerpt) are important for search engine optimization (SEO) and thus Reguide encourages to include these in blog posts.

`slug` is the unique identifier of the url. Thus, `slug` must be unique and different from other posts. The whitespace of `slug` needs to be separated with `-` or `_` but `-` is recommended. However, even if you don't write the correct slug, Reguide will automatically slugify your incorrect slug. If slug is not specified, the slugified title of the post will be used as slug.
...
Here are some recommendations, tips & ticks for creating new posts in Reguide blog theme.
...
There's one thing to note about headings. The Reguide blog posts use title (title in the frontmatter) as the main heading of the post. Therefore, the rest of the heading in the post should be using h2 \~ h6.
